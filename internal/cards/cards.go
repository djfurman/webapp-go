...
// GetPaymentMethod gets the payment method by payment intent ID and returns it
func (c *Card) GetPaymentMethod(paymentIntentID string) (*stripe.PaymentMethod, error) {
	stripe.Key = c.Secret

	pm, err := paymentmethod.Get(paymentIntentID, nil)
	if err != nil {
		return nil, err
	}
...
// RetrievePaymentIntent gets the payment intent's current state by Payment Intent ID and returns it
